...
class TabAcquireEMGData(QWidget):
    ...
    def _start_acquisition(self):
        ...
        params = {
            "serial_port": self.combo_serial.currentText(),
            "baud_rate": 115200
        }
        ...
        self.toast = ToastMessage(self, self.tr("Starting acquisition with board {0}").format("ESP32"), "#0077cc")
        ...
        self.worker = AcquisitionWorkerEMG(params=params, protocol_path=protocol_path, profile_path=profile_path, mode=mode_id)
    ...
    def status_controller(self, status, code):
        if status == 1:
            self.toast = ToastMessage(
                self,
                self.tr("Acquisition started"),
                SUCCESS_COLOR)
        elif status == 2:
            self.toast = ToastMessage(
                self,
                self.tr("Acquisition completed"),
                SUCCESS_COLOR)
            self._restore_ui()
        elif status == -1:
            self.toast = ToastMessage(
