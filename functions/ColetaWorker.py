...
class ColetaWorker(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            self.canais = self.user_data['canais'].keys()
            header = ['timestamp'] + list(self.canais) + ['events']
            ...
            while self._is_running:
                ...
                dados = board.get_current_board_data(BUFFER_TAMANHO)
                for i in range(dados.shape[1]):
                    amostra = dados[:, i]
                    with open(self.arquivo_nome, "a", newline="") as f:
                        csv.writer(f).writerow(amostra)
                    self.nova_amostra.emit(amostra.tolist())
